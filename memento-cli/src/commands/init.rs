...
fn render_daemon_config(plan: &InitPlan) -> String {
    let transport = if plan.preset == Preset::Windows {
        format!(
            "transport = \"named_pipe\"\npipe_name = \"{}\"\n",
            toml_string(&memento_ipc::windows_pipe_name_for_path(&plan.data_dir))
        )
    } else {
        format!(
            "transport = \"unix\"\nsocket_path = \"{}\"\n",
            toml_path(&memento_ipc::unix_socket_path(&plan.data_dir))
        )
    };
    format!(
        concat!(
...
fn resolve_vault_sync_runner() -> VaultSyncRunnerPlan {
    ...
    if let Some(command) = find_command("memento-vault-sync") {
        return VaultSyncRunnerPlan {
            command: vec![command.display().to_string()],
            working_dir: None,
        };
    }
    ...
    for dir in std::env::split_paths(&path_var) {
        ...
        if let Some(extensions) = std::env::var_os("PATHEXT") {
            for extension in std::env::split_paths(&extensions) {
                let candidate = dir.join(format!("{}{}", command, extension.to_string_lossy()));
...
mod tests {
    ...
    fn vault_sync_config_renders_connectors() {
        ...
        .unwrap();
        plan.documents_root = Some(tmp.path().join("Documents"));
